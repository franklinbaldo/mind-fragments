...
title: Dynamic OG image generation in Mind Fragments blog posts
...
tags:
  - docs
  - release
description: Dynamic OG image generation for Mind Fragments blog posts, based on the feature from AstroPaper v1.4.0.
---

Mind Fragments supports dynamic OG image generation for blog posts (based on the AstroPaper v1.4.0 feature).
...
The original theme, AstroPaper, already provided a way to add an OG image to a blog post. The author can specify the OG image in the frontmatter `ogImage`. Even when the author doesn't define the OG image in the frontmatter, the site's default OG image (via SITE.ogImage) will be used as a fallback. But the problem is that the default OG image is static, which means every blog post that does not include an OG image in the frontmatter will always use the same default OG image despite each post title/content being different from others.
...
In Mind Fragments, Vercel's [Satori](https://github.com/vercel/satori) package is used for dynamic OG image generation.
...
## Anatomy of Dynamic OG Image in Mind Fragments

The dynamic OG image in Mind Fragments includes _the blog post title_, _author name_ and _site title_. Author name and site title will be retrieved via `SITE.author` and `SITE.title` of **"src/config.ts"** file. The title is generated from the blog post frontmatter `title`.
